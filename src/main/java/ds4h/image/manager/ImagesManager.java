...
public class ImagesManager implements ListIterator<ImagePlus>, Observable {
    ...
    private BufferedImage getImage(int index, boolean wholeSlide, boolean isOriginal) {
        ...
        for (ImageFile file : imageFileList) {
            if (progressive + file.getNImages() > index) {
                imageFile = file;
                break;
            }
            progressive += file.getNImages();
        }
        try {
            if (index == -1) {
    ...
    public List<RoiManager> getRoiManagers() {
        return this.getImageFiles().stream().map(ImageFile::getRoiManagers).flatMap(List::stream).collect(Collectors.toList());
    }
    ...
    public void removeImageFile(int index) {
        ...
        this.imageIndex = this.getImageIndex() >= this.getNImages() ? this.getImageIndex() - 1 : this.getImageIndex();
    }
